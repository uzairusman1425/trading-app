...
function secondsToDateString(seconds) {
	...
	const dateInMilliseconds = seconds * 1000

	// Create a new Date object representing the desired date (October 19, 2018)
	const targetDate = new Date(2018, 9, 19) // Year, Month (0-indexed), Day

	// Add the number of seconds to the target date
	const newDate = new Date(targetDate.getTime() + dateInMilliseconds)

	// Format the date as YYYY-MM-DD
	const year = newDate.getFullYear()
	const month = String(newDate.getMonth() + 1).padStart(2, "0") // Add leading zero for single-digit months
	const day = String(newDate.getDate()).padStart(2, "0") // Add leading zero for single-digit days
...
export default function Trading() {
	...
	useEffect(() => {
		const ws = new WebSocket("wss://marketdata.tradermade.com/feedadv") // Replace with actual URL

		ws.onopen = () => {
			setConnection(ws)
			console.log("WebSocket connection opened")
			ws.send(
				`{"userKey":"wsyUli6SABrt7bXhzRcQ", "symbol":"${selectedPair}"}`
			)
		}
		...
		return () => ws.close() // Cleanup function to close connection on unmount
	}, [])
